  ...
  const runOcrMatch = async () => {
    ...
    try {
      ...
      if (text.includes(expectedId)) {
        setStatus('matched');
        setMessage(`✅ Identity verified! ID (${username}) matched.`);
      } else {
        setStatus('failed');
        setMessage(`❌ Could not find ID (${username}) on the card. Please ensure the text is clear and readable.`);
      }
    } catch (err) {
      setStatus('error');
  ...
  return (
    <div style={{ animation: 'slideUp 0.3s ease' }}>
      ...
      <p style={{ fontSize: 13, color: '#475569', textAlign: 'center', marginBottom: 20, lineHeight: 1.5 }}>
        Scan your <strong>School/Teacher ID card</strong>. Our system will read the text to verify your ID.
      </p>
...
export default function LoginPage() {
  ...
  const handleLoginSubmit = async (e, customUser, customPwd) => {
    ...
    try {
      ...
      if (res.data.requiresIdVerification && loginRole !== 'Vendor') {
      ...
      setError(err.response?.data?.message || (loginRole === 'Vendor' ? 'Incorrect passkey or password.' : 'Incorrect Student/Teacher ID or password.'));
    } finally {
      setLoading(false);
    }
  };
          ...
          >
            {verificationStep && pendingCredentials ? (
              ...
              <>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <h3 style={{ fontSize: 18, fontWeight: 800, color: loginRole === 'Vendor' ? '#166534' : '#15803D', margin: 0 }}>
                      {loginRole === 'Vendor' ? 'Canteen Vendor Login 🏪' : 'User / Customer Login 🎓'}
                    </h3>
                    {loginRole !== 'Vendor' && (
                ...
                {/* Step indicator for users/students/teachers */}
                {loginRole !== 'Vendor' && (
                  <div style={{ height: 4, background: '#E2E8F0', borderRadius: 4, marginBottom: 20, overflow: 'hidden' }}>
                ...
                <form onSubmit={(e) => handleLoginSubmit(e)}>
                  <div className="input-group">
                    <label>{loginRole === 'Vendor' ? 'Vendor Passkey' : 'Student / Teacher ID Number'}</label>
                    <input
                      className="input"
                      type="text"
                      placeholder={loginRole === 'Vendor' ? 'Enter your vendor passkey' : 'e.g. 2026-00125 or T-2026-00125'}
                      value={username}
                  ...
                  {loginRole !== 'Vendor' && (
                    <div style={{
                      ...
                      <span>After credentials are verified, you'll be asked to scan your <strong>School/Teacher ID card</strong> for identity confirmation.</span>
                    </div>
                  )}
                  {loginRole === 'Vendor' && (
                    <div style={{
                  ...
                  <div style={{ textAlign: 'center', marginTop: 18, fontSize: 13, color: '#64748B' }}>
                    {loginRole === 'Vendor' ? (
                      ...
                      <p>Don't have an account? <Link to="/register" style={{ color: '#15803D', fontWeight: 700, textDecoration: 'none' }}>Register Student/Teacher Account</Link></p>
                    )}
                  </div>
                </form>
              </>
